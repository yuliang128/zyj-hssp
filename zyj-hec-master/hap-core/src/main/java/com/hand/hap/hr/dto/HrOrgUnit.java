...
public class HrOrgUnit extends BaseDTO {
    ...
    @Column
    @Where
    @JoinTable(name = "unitJoin", joinMultiLanguageTable = true, target = HrOrgUnit.class, type = JoinType.LEFT,
                    on = {@JoinOn(joinField = HrOrgUnit.FIELD_UNIT_ID),
                            @JoinOn(joinField = BaseDTO.FIELD_LANG, joinExpression = BaseConstants.PLACEHOLDER_LOCALE)})
    @JoinTable(name = "unitJoin2", target = HrOrgUnit.class, type = JoinType.LEFT,
                    on = {@JoinOn(joinField = HrOrgUnit.FIELD_UNIT_ID)})
    @OrderBy
    ...
    @JoinTable(name = "positionJoin", joinMultiLanguageTable = true, target = Position.class, type = JoinType.LEFT,
                    on = {@JoinOn(joinField = Position.FIELD_POSITION_ID),
                            @JoinOn(joinField = BaseDTO.FIELD_LANG, joinExpression = BaseConstants.PLACEHOLDER_LOCALE)})
    @OrderBy
    private Long managerPosition;

    @JoinTable(name = "companyJoin", joinMultiLanguageTable = true, target = Company.class, type = JoinType.LEFT,
                    on = {@JoinOn(joinField = Company.FIELD_COMPANY_ID),
                            @JoinOn(joinField = BaseDTO.FIELD_LANG, joinExpression = BaseConstants.PLACEHOLDER_LOCALE)})
    @JoinTable(name = "companyJoin2", target = Company.class, type = JoinType.LEFT,
                    on = {@JoinOn(joinField = HrOrgUnit.FIELD_COMPANY_ID)})
    @OrderBy
