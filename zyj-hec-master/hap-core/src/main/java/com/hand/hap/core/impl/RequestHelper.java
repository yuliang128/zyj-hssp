...
public final class RequestHelper {
    ...
    public static IRequest createServiceRequest(HttpServletRequest httpServletRequest) {
        ...
        if (session != null) {
            ...
            if (session.getAttribute(FndCompany.FIELD_COMPANY_ID) != null) {
                requestContext.setCompanyId((Long) session.getAttribute(FndCompany.FIELD_COMPANY_ID));
            }
            if (session.getAttribute(User.FIELD_EMPLOYEE_ID) != null) {
                requestContext.setEmployeeId((Long) session.getAttribute(User.FIELD_EMPLOYEE_ID));
            }
            if (session.getAttribute(IRequest.EMP_CODE) != null) {
                requestContext.setEmployeeCode((String) session.getAttribute(IRequest.EMP_CODE));
            }
            if (session.getAttribute(IRequest.EMP_NAME) != null) {
                requestContext.setEmployeeName((String) session.getAttribute(IRequest.EMP_NAME));
            }
            if (session.getAttribute(FndCompany.FIELD_MAG_ORG_ID) != null) {
                requestContext.setMagOrgId((Long) session.getAttribute(FndCompany.FIELD_MAG_ORG_ID));
            }
            if (session.getAttribute(IRequest.FILED_DB_TYPE) != null) {
                requestContext.setDbType((String) session.getAttribute(IRequest.FILED_DB_TYPE));
            }
            if (session.getAttribute(IRequest.FIELD_SESSION_ID) != null) {
                requestContext.setSessionId((Long) session.getAttribute(IRequest.FIELD_SESSION_ID));
            }
            ...
            if (authentication instanceof OAuth2Authentication) {
                ...
                if (oauth2Authentication.getUserAuthentication() != null) {
                    ...
                    if (extensions.get(FndCompany.FIELD_COMPANY_ID) != null) {
                        requestContext.setCompanyId(Long.valueOf(extensions.get(FndCompany.FIELD_COMPANY_ID).toString()));
                    }
                    if (session.getAttribute(FndCompany.FIELD_MAG_ORG_ID) != null) {
                        requestContext.setMagOrgId((Long) session.getAttribute(FndCompany.FIELD_MAG_ORG_ID));
                    }
