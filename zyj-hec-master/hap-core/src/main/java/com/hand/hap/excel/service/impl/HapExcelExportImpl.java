...
public class HapExcelExportImpl {
     ...
     * @throws IOException
     */
    private void createSheetTemplate(Class dto, boolean containStdWho, List<String> columns, String way) throws IOException {
        EntityTable tableEntity = TableUtils.getTable(dto);
        SXSSFSheet sheet = createSheet(tableEntity.getName());
        sheet.setDefaultColumnWidth(20);
        List<String> column;
        if (way == null) {
            column = TableUtils.getColumn(tableEntity, containColumns, containStdWho);
        } else {
            column = columns.stream().map(c -> {
     ...
     * @throws IOException
     */
    public void createExcelTemplate(String tableName, boolean containStdWho, List<String> columns, String way) throws IOException {
        ...
        for (Class tableDto : allTables) {
            createSheetTemplate(tableDto, containStdWho, columns, way);
        }
    }
     ...
     * @throws IOException
     */
    public void createExcelTemplate(String tableName, List<String> columns, String way) throws IOException {
        createExcelTemplate(tableName, true, columns, way);
    }
