...
# 取出properties文件信息
prop(){
    grep "${1}" webapps/zyjhec/WEB-INF/classes/config.properties | cut -d'=' -f2 | sed 's/\r//'
}
...
# 替换Redis数据库链接数据
sed -i 's/^redis.ip=.*$/redis.ip='${REDIS_IP}'/g' webapps/zyjhec/WEB-INF/classes/config.properties
sed -i 's/^redis.port=.*$/redis.port='${REDIS_PORT:-"6379"}'/g' webapps/zyjhec/WEB-INF/classes/config.properties
sed -i 's/^redis.db=.*$/redis.db='${REDIS_DB:-"8"}'/g' webapps/zyjhec/WEB-INF/classes/config.properties
