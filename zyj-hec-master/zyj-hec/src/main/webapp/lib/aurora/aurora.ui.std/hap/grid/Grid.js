...
//        sf.overId = NULL;
    ...
    initComponent:function(config){
        ...
        sf.initTemplate();
        // if(!Ext.isIE8){
        // 	$("#" + sf.id).find("div.grid-nicescroll-box").niceScroll(".grid-nicescroll-wrap");
        //     $("#" + sf.id).find(".grid-ub").niceScroll({enablemousewheel:false});
        // }
        flag = false;
    },
...
//        Ext.getBody()[ou]('mousewheel',sf.onBodyMouseWheel,sf);
        sf.wb[ou](Ext.isOpera ? "keypress" : EVT_KEY_DOWN, sf.handleKeyDown,  sf)
            [ou]("keyup", sf.handleKeyUp,  sf)
            [ou](Ext.isIE || isIE11? "focusin":"focus",sf.onFocus,sf)
            [ou](Ext.isIE || isIE11? "focusout":"blur",sf.onBlur,sf);
        sf.ub[ou]('scroll',sf.syncScroll, sf)
            [ou](EVT_CLICK,sf.onClick, sf)
            [ou]("focus",sf.onUbFocus,sf)
            [ou]("blur",sf.onUbBlur,sf)
        ...
        sf.body[ou]('click',sf.bodyClick,sf);
    ...
    /*
     * 由于grid中的autoAdjustHeight为true时，造成grid没有高度，所以使用nicescroll滚动插件后页面没有出现滚动条
     * 2018-02-01  向平
     */
...
//        }
    ...
    bodyUpdate : function(){
    	...
    	// if(!Ext.isIE8){
        //     setTimeout(function(){
		// 		$("#" + sf.id).find("div.grid-ub").getNiceScroll().resize();
    ...
    creatListView : function(target){
    	...
    	Ext.onReady(function(){
        	...
        	filter.push('<div class="filter-title">显示符合以下条件的行</div>'+
        		    	'<div class="first-select"><select class="select1">' +
        		    	'<option value="等于">等于</option><option value="不等于">不等于</option><option value="开头为">开头为</option>'+
        	            '<option value="包含">包含</option><option value="不包含">不包含</option><option value="结尾为">结尾为</option><option value="is null">is null</option>'+
        	            '<option value="is not null">is not null</option><option value="is empty">is empty</option><option value="is not empty">is not empty</option>'+
        	            '</select></div><input type="text" class="input-value first-input"/>' +
        	            '<div class="first-select second-select"><select class="select2"><option value="并且">并且</option><option value="或者">或者</option></select></div>'+
        	            '<div class="first-select last-select"><select class="select3">' +
        		    	'<option value="等于">等于</option><option value="不等于">不等于</option><option value="开头为">开头为</option>'+
        	            '<option value="包含">包含</option><option value="不包含">不包含</option><option value="结尾为">结尾为</option><option value="is null">is null</option>'+
        	            '<option value="is not null">is not null</option><option value="is empty">is empty</option><option value="is not empty">is not empty</option>'+
        	            '</select></div><input type="text" class="input-value second-input"/>' +
        	            '<div class="btn-wrap"><button class="filter-btn" value="过滤">过滤</button><button class="clear-btn" value="清除">清除</button></div>')
        	sf.st =new Ext.Template( "<div class='sort-wrap'><ul class='sort-box'><li class='ascending'><span class='ascend-icon'></span><span>升序</span></li>"+
        		"<li class='descending'><span class='descend-icon'></span><span>降序</span></li>"+
        		"<li class='column' style='position:relative;border-top:1px solid #ccc;'><span class='list-icon'></span><span>列</span><span class='list-expend'></span> "+list.join('')+"</li>" +
        		"<li class='filter' style='position:relative;border-top:1px solid #ccc;'><span class='filter-icon'></span><span>过滤</span><span class='filter-expend'></span> "+filter.join('')+"</li>" +
        				"</ul></div>").insertFirst(document.body,{shadow:Ext.isIE?'<div class="item-ie-shadow"></div>':''},true);
    	});
//    	$(".list-wrap").niceScroll();
    ...
    onfilterBtnClick : function(e,t,target){
    	var sf = this;
    	    isFilter = false,
    		td = target.parent("td"),
    		tdDisplay = td.getAttribute("dataindex"),
    		ds = sf.dataset,
    		datas = ds.data,
    		selectOne = sf.st.child(".select1"),
    		selectTwo = sf.st.child(".select2"),
    		selectThree = sf.st.child(".select3"),
    		inputOne = sf.st.child(".first-input"),
    		inputTwo = sf.st.child(".second-input"),
    		inputValue1 = inputOne.getValue(),
    		inputValue2 = inputTwo.getValue(),
    		selectValue1 = selectOne.getValue(),
    		selectValue2 = selectTwo.getValue(),
    		selectValue3 = selectThree.getValue();
...
//    			sf.filterOperation(selectValue1,datas,inputValue1,tdDisplay);
    		...
    		for(var i=0;i<datas.length; ){
	    		...
	    		}else if(selectValue1 === "is not null" || selectValue3 === "is not null"){
	    			if(datas[i].data[tdDisplay] === "null"){
	    				datas.remove(datas[i]);
	    			}else{
	    				i++
	    			}
	    		}else if(selectValue1 === "is empty" || selectValue3 === "is empty"){
	    			if(datas[i].data[tdDisplay] !== "" || datas[i].data[tdDisplay] !== undefined){
	    				datas.remove(datas[i]);
	    			}else{
	    				i++
	    			}
	    		}else if(selectValue1 === "is not empty" || selectValue3 === "is not empty"){
	    			if(datas[i].data[tdDisplay] === "" || datas[i].data[tdDisplay] === undefined){
	    				datas.remove(datas[i]);
	    			}else{
	    				i++
    ...
    orFilter : function(datas,i,tdDisplay,inputValue2){
		...
		}else if(selectValue3 === "等于"){
			if(datas[i].data[tdDisplay]===inputValue2){
				return true;
			}else{
				return false;
			}
		}else if(selectValue3 === "不等于"){
			if(datas[i].data[tdDisplay]!==inputValue2){
				return true;
			}else{
				return false;
			}
		}else if(selectValue3 === "包含"){
			if(datas[i].data[tdDisplay].indexOf(inputValue2) != -1){
				return true;
			}else{
				return false;
			}
		}else if(selectValue3 === "不包含"){
			if(!(datas[i].data[tdDisplay].indexOf(inputValue2) != -1)){
				return true;
			}else{
				return false;
...
//                ds.setQueryParameter('ORDER_TYPE', 'desc');
    ...
    columnPosition : function(sf,wrap){
		...
		screenWidth = $A.getViewportWidth()-20,
		...
		if(screenHeight > y + listH && screenWidth > x + listW){
	    	...
	    	wrap.moveTo(x,y);
		}else if(screenHeight < y + listH && screenWidth > x + listW){
			y = y + columnH/2;
    	...
    	if(sortWrap){
    		...
    		if((x == stLeft) || ((x == stLeft + stW + eleX - screenWidth) && eleX + stW > screenWidth)){
        		...
        		if(screenHeight > stHeight + y && screenWidth > stWidth + x){
    	        	...
    	        	sf.st.moveTo(x,y);
    			}else if(screenHeight < stHeight + y && screenWidth > stWidth + x){
    				y = y - eleH -10;
...
//    	$(".list-wrap").getNiceScroll().resize();
    },
...
//                var row;
    ...
    bind : function(ds,fl){
        ...
        if(ds.autopagesize===TRUE && ds.fetchall!==TRUE){
            ds.pagesize=Math.max(10,Math.floor(((parseInt(('' + sf.ub.getStyle('max-height')).replace('px','')) || parseInt(('' + sf.ub.getStyle('height')).replace('px','')) || sf.ub.getHeight())||parseFloat(sf.ub.dom.style.height))/ROW_HEIGHT) -1);
...
//            width:col.width-2,
    ...
    createCell : function(col,record,td,rowSpan){
        ...
        if(xtype == ROW_CHECK || xtype == ROW_RADIO){
            ...
            /*
             * 不判断节点是否新增以及当前值是否为空，只要当前字段时新增，且字段必输就携带item-notBlank class
             */
            //if(field && IS_EMPTY(record.data[col.name]) && record.isNew == TRUE && field.get(REQUIRED) == TRUE){
//            debugger;
            /*
             * 徐昭 2017/11/07 17:19
             * 不管当前节点是否新增，只要当前字段值为空，且字段必输就携带item-notBlank class
             */
//            if(field && record.isNew === true && field.get(REQUIRED) == TRUE){
...
//                    width:data.width-sp,//-11
    ...
    onBeforeRemove : function(){
        A.Masker.mask(Ext.getBody(),_lang['grid.mask.remove']);
    },
    onWait : function(){
    	if(this.submask == TRUE)
        A.Masker.mask(Ext.getBody(),_lang['grid.mask.waiting']);
    },
    onBeforeLoad : function(){
        this.ub.scrollTo(LEFT,0);
        this.uh.scrollTo(LEFT,0);
        if(this.submask == TRUE)
        A.Masker.mask(Ext.getBody(),_lang['grid.mask.loading']);
    },
    onBeforSubmit : function(ds){
        if(this.submask == TRUE)
        A.Masker.mask(Ext.getBody(),_lang['grid.mask.submit']);
    },
    onFetching : function(ed){
    	var sf = this;
    	sf.isFetching = TRUE;
    	A.Masker.mask(Ext.getBody(),_lang['grid.mask.fetching']);
...
//    },
    onMouseWheel : function(e){
        ...
        //去掉了当前grid是否focus的判断，在IE11上存在问题
        if(sf.editing == TRUE||!ds.getAll().length) return;
        if(!sf.hasFocus){
        	// if(!Ext.isIE8){
        	// 	var ns =$("#" + sf.id).find(".grid-ub").getNiceScroll(0);
        	// 	ns.doScrollTop(ns.scrollTop() + delta * (-20));
        	// }
//        	sf.onBodyMouseWheel(e);
        }else{
	    	e.stopEvent();
	    	if(delta > 0){
	    		ds.pre();
...
//		if(t.parent("div.grid-uh")){
//			var gridUh = t.parent("div.grid-uh");
//			var startIndex = t.dom.cellIndex;
//	        var endIndex;
//	        var gridUb = sf.wrap.child("div.grid-ub");
//	        flag = true;
////	        $('body').addClass('no-select-text');
//	        $(document).mousemove(function(e) {
//	            if (flag) {
//	                var even = e || window.event;
//	                var x = even.clientX + 5 + 'px';
//	                var y = even.clientY + 5 + 'px';
...
//
//	        });
//	        $("#" + sf.id).find('.grid-uh table td').mouseenter(function () {
//	            endIndex = $(this).index();
//	            if (endIndex == startIndex) {
//	            	$("#" + sf.id).find('#triangle').css('display', 'none');
//	            } else {
//	            	$("#" + sf.id).find('#triangle').css('display', 'block');
//	            }
//	            var offsetW = 0;
...
//	            $("#" + sf.id).find('#triangle').css('display', 'none');
//	            if (endIndex < startIndex) {
//	            	$("#" + sf.id).find(".grid-ub table tr").each(function (i) {
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(' + i + ') td:eq(' + endIndex + ')').before($("#" + sf.id).find('.grid-ub table tr:eq(' + i + ') td:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(' + i + ') td:eq(' + (startIndex + 1) + ')').remove();
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(' + i + ') td:eq(' + endIndex + ')').before($("#" + sf.id).find('.grid-uh table tr:eq(' + i + ') td:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(' + i + ') td:eq(' + (startIndex + 1) + ')').remove();
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(0) th:eq(' + endIndex + ')').before($("#" + sf.id).find('.grid-ub table tr:eq(0) th:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(0) th:eq(' + (startIndex + 1) + ')').remove();
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(0) th:eq(' + endIndex + ')').before($("#" + sf.id).find('.grid-uh table tr:eq(0) th:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(0) th:eq(' + (startIndex + 1) + ')').remove();
//	                });
//	            	
//	            } else if (endIndex > startIndex) {
//	            	$("#" + sf.id).find(".grid-ub table tr").each(function (i) {
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(' + i + ') td:eq(' + endIndex + ')').after($("#" + sf.id).find('.grid-ub table tr:eq(' + i + ') td:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(' + i + ') td:eq(' + (startIndex) + ')').remove();
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(' + i + ') td:eq(' + endIndex + ')').after($("#" + sf.id).find('.grid-uh table tr:eq(' + i + ') td:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(' + i + ') td:eq(' + (startIndex) + ')').remove();
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(0) th:eq(' + endIndex + ')').after($("#" + sf.id).find('.grid-ub table tr:eq(0) th:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-ub table tr:eq(0) th:eq(' + (startIndex) + ')').remove();
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(0) th:eq(' + endIndex + ')').after($("#" + sf.id).find('.grid-uh table tr:eq(0) th:eq(' + startIndex + ')').clone(true));
//	            		$("#" + sf.id).find('.grid-uh table tr:eq(0) th:eq(' + (startIndex) + ')').remove();
//	                });
//
//	            }
//	            $(document).unbind("mousemove");
//	            $(document).unbind("mouseup");
//	            $("#" + sf.id).find('.grid-uh table td').unbind("mouseenter");
//	        });
//		}
    },
    focus: function(){
    ...
    onFocus : function(){
    	var sf = this;
    	this.hasFocus = TRUE;
    	var layoutForm = sf.wrap.parent(".layout-form");
    	if(!sf.wrap.hasClass("gridShine")&&!layoutForm){
...
//        var v = 0;
    ...
    renderEditor : function(div,record,c,editor){
        ...
        }else if(editor != _N || ($au(editor) instanceof CheckBox)){
    ...
    onUpdate : function(ds,record, name, value,oldvalue){
        ...
        if(div = Ext.get([sf.id,name,record.id].join(_))){
            ...
            if(editor!=_N ? ($au(editor) instanceof CheckBox):(name && !IS_EMPTY(record.getField(name).get(CHECKED_VALUE)))){
...
//        }
    ...
    setEditor: function(name,editor){
        ...
        if(div){
            if(editor == _N){
                div.removeClass(CELL_EDITOR);
            }else if(!$au(editor) instanceof CheckBox){
                div.addClass(CELL_EDITOR);
            }
        }
    },
...
//                    ed.setWidth(p.getWidth()-7);
                    ...
                    if(ed.el.hasClass("item-img-textField") && v){
...
//            sf.currentEditor = NULL;
        ...
        // 徐昭 2017/11/1 14:20
        // 防止editor失焦点时，鼠标grid之外，grid仍处于选中状态
//        if(sf.wrap.hasClass("gridShine")){
//        	sf.wrap.removeClass("gridShine");
//        }
    },
...
//              }
    ...
    setWidth: function(w){
        ...
        sf.width = w;
        sf.wrap.setWidth(w+2);//增加border的宽度
        ...
        /**
         * author:徐昭
         * email:zhao.xu@hand-china.com
         * time:2017/09/01
         * purpose:当navbar和toolbar同时存在时，宽度减半
         */
        if(tb && nb){
        	tb.setWidth(w/2);
        	nb.setWidth(w/2);
...
//        var bh = h - 25;
    	...
    	// 	$("div.grid-ub").getNiceScroll().remove();
    	// 	$("div.grid-ub").niceScroll({enablemousewheel:false});
    	// 	setTimeout(function(){
        //         $("div.grid-ub").getNiceScroll().resize();
		// 	},100)
    	// }
...
//          }
    ...
    showExportConfirm :function(){
        var sf = this,n=0,id = sf.id + '_export',
            msg = ['<div class="item-export-wrap" style="margin:10px;width:270px;position:relative;" id="',id,'">',
                    ...
                    '</tr></tbody></table></div>',
                    '<div class="exportBox" style="overflow:auto;height:200px;"><div class="exportNiceScroll"><table cellSpacing="0" cellPadding="0" border="0"><tbody>'],
        ...
        // if(!Ext.isIE8){
        // 	$(".exportBox").niceScroll(".exportNiceScroll");
        // }
    },
...
//    	debugger;
    	var sf = this,
    		...
    		url = ctx + 'sys_spreadSheet.screen?bind_id='+ bind_id;
    	sf.isWinOpen = true;
    	sf.win = new A.Window({id:id,title:sf.title||'快速表格',url:Ext.urlAppend(url),width:1000,height:600,fullScreen:true});
