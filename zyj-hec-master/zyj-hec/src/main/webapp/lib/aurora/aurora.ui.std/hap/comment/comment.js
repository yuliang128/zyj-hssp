$A.Comment = Ext.extend($A.Component, {
	tpl : [
			...
			"</div>",
			"<div class='comment-bar'><a href='javascript:$au(\"{id}\").remove(\"{commentid}\")'>删除</a></div>",
			"</div>", "</li>"],
	maskTpl : [
			"<div class='comment-login-mask' >",
			"<div unselectable='on' class='comment-mask'></div>",
			"<div unselectable='on' class='comment-login'><p>匿名用户不能发表评论</p><p><a href='javascript:$au(\"{id}\").login()'>登录</a> | <a href='javascript:$au(\"{id}\").register()'>注册</a></p></div>",
			"</div>"],
	initComponent : function(config) {
		$A.Comment.superclass.initComponent.call(this, config);
		this.txt = $au(this.id + "_txt");
