...
$A.usedHeight = 20;// -6
$A.usedWidth = 20;// -6
$A.minViewportWidth = 1000;
$A.minViewportHeight = 500;
$A.auWindow = new Ext.util.Observable();
...
//				var value = name + "=aurora-" + $A.encryptByDES(arr[i].substr(num+1),key);
//			}
//			if(str==""){
//				str += value;
//			}else{
...
//}
//
//$A.encryptByDES = function(message,key){
 ...
 *
 * @param {Object/String}
 *            el Aurora组件对象或者是DOM对象或者是对象的ID字符串
 */
...
Ext.Ajax.on("requestexception", function (conn, response, options) {
    ...
    if ($A.logWindow) {
        var record = $au('HTTPWATCH_DATASET').getCurrentRecord();
        var st = $A['_startTime'];
        var ed = new Date();
...
/**
 * 获取Aurora控件的对象，可以使用简写方式的$au()方法
 *
 * @param {String}
 *            id Aurora控件的id
 */
$au = $A.getCmp = function (id) {
...
$A.getViewportHeight = function () {
    var viewportHeight = $A.minViewportHeight;
    ...
    if (viewportHeight < $A.minViewportHeight) {
        viewportHeight = $A.minViewportHeight
    }
...
$A.getViewportWidth = function () {
    var viewportWidth = $A.minViewportWidth;
    if (Ext.isIE || Ext.isIE9 || Ext.isIE10) {
    ...
    if (viewportWidth < $A.minViewportWidth) {
        viewportWidth = $A.minViewportWidth;
    }
...
$A.request = function (opt) {
    var url = opt.url,
        isRest = true || /\/rest\//.test(url) || opt.isRest,
        para = opt.para,
        xmlData = opt.xmlData,
    ...
    if ($A.logWindow) {
        ...
        $au('HTTPWATCH_DATASET').create({'url': url, 'request': Ext.util.JSON.encode({parameter: para})})
    }
    if (isRest) {
        if (method != 'GET') {
        ...
        success: function (response, options) {
            ...
            if (response) {
                if ($A.logWindow) {
                    ...
                    var record = $au('HTTPWATCH_DATASET').getCurrentRecord();
                    record.set('spend', ed - st);
                    record.set('rows', response.statusText);
                    record.set('status', response.status);
                ...
                if (Ext.isEmpty(response.responseText)) {
                    successCall.call(scope, {rows: {}}, options);
                } else {
                    try {
                    ...
                    if (res) {
                        ...
                        if (!res.success) {
                            ...
                            if (res.code && (res.code == 'session_expired' || res.code == 'login_required')) {
                                if ($A.manager.fireEvent('timeout', $A.manager))
                                    $A.showErrorMessage(_lang['ajax.error'], _lang['session.expired']);
                            } else {
                                var st = res.stackTrace;
                                st = (st) ? st.replaceAll('\r\n', '</br>') : '';
                                if (res.message) {
                                    var h = (st == '') ? 150 : 250;
                                    $A.showErrorMessage(_lang['ajax.error'], res.message + '</br>' + st, null, 400, h);
                                } else {
...
// this.shadow.setHeight(this.tip.getHeight())
        ...
        correctPosition: function (ele) {
        	 ...
        	 * 减去20是body得padding值
        	 */
            var screenWidth = $A.getViewportWidth() - 20;
            var screenHeight = $A.getViewportHeight();
            var x = ele.getX();
//            var sx = ele.getX() - 2;
            /*
...
// this.hide();
            ...
            if ($A.PARENT_DOMAIN && parent.showSideBar) {
                ...
                this.bar.setStyle('left', '50%');
                this.bar.setStyle('margin-left', '-' + this.bar.getHeight() / 2 + 'px');
                this.bar.fadeIn();
...
$A.Cover = function () {
    var m = {
        ...
        container: {},
        cover: function (el) {
// if(!$A.Cover.bodyOverflow)$A.Cover.bodyOverflow =
...
$A.Masker = function () {
    var m = {
        ...
        mask: function (el, msg) {
            ...
            if (el === Ext.getBody()) {
                w = $A.getViewportWidth() - 2;
                h = $A.getViewportHeight() - 2;
                vh = Math.min(h - 2, 30);
                zi = 99999;
            }
            if (msg === _lang['mask.loading'] || msg === _lang['grid.mask.loading'] || msg === _lang['lov.query']) {
                var p = '<div class="aurora-mask aurora-loading-mask"  style="left:-10000px;top:-10000px;width:' + w + 'px;height:' + h + 'px;position: absolute;"><div unselectable="on"></div><span class="loading-icon" style="top:' + (h / 2 - 11) + 'px;left:' + (w - 100) / 2 + 'px;"></span><span style="font-size:16px;top:' + (h / 2 + 50) + 'px;left:' + (w - 100) / 2 + 'px;">' + msg + '</span></div>';
            } else if (msg == _lang['grid.mask.submit']) {
                var p = '<div class="aurora-mask aurora-submit-mask"  style="left:-10000px;top:-10000px;width:' + w + 'px;height:' + h + 'px;position: absolute;"><div unselectable="on"></div><span class="submit-icon" style="top:' + (h / 2 - 11) + 'px;left:' + (w - 100) / 2 + 'px;"></span><span style="font-size:16px;top:' + (h / 2 + 50) + 'px;left:' + (w - 100) / 2 + 'px;">' + msg + '</span></div>';
            } else {
                var p = '<div class="aurora-mask aurora-other-mask"  style="left:-10000px;top:-10000px;width:' + w + 'px;height:' + h + 'px;position: absolute;"><div unselectable="on"></div><span class="other-icon" style="top:' + (h / 2 - 11) + 'px;left:' + (w - 100) / 2 + 'px;"></span><span style="font-size:16px;top:' + (h / 2 + 50) + 'px;left:' + (w - 100) / 2 + 'px;">' + msg + '</span></div>';
            }
...
$A.getRenderer = function (renderer) {
    ...
    /*
	 * var rder; if(renderer.indexOf('Aurora.') != -1){ rder =
	 * $A[renderer.substr(7,renderer.length)] }else{ rder = window[renderer]; }
	 */
    try {
...
$A.showValidWindowMsg = function (ds) {
    ...
    for (var i = 0; i < rs.length; i++) {
        ...
        sb[sb.length] = _lang['valid.fail.note'] + '<a href="#" onclick="$au(\'' + r.ds.id + '\').locate(' + index + ')">(' + r.id + ')</a>:';
...
// if(!$A.logWindow) {
// $A.logWindow = new $A.Window({modal:false,
...
$A.DataSet = Ext.extend(Ext.util.Observable, {
    constructor: function (config) {// datas,fields, type
        ...
        this.totalcountfield = config.totalcountfield || 'total';
        ...
        if (this.bindtarget && this.bindname) this.bind($au(this.bindtarget), this.bindname);// $au(this.bindtarget).bind(this.bindname,this);
        this.qds = Ext.isEmpty(config.querydataset) ? null : $au(config.querydataset);
...
// if(this.sortInfo) this.sort();
        ...
        if (this.bindtarget && options) {
            ...
            //2018-08-28 添加options.opts是否存在判断 gyt
            if (options.opts && options.opts.record && cr != options.opts.record) {
        ...
        if (res.rows) {
            ...
            if (this.bindtarget) {
                ...
                if (bd.getCurrentRecord() == options.opts.record) {
                    ...
                    for (var i = 0; i < datas.length; i++) {
                        var data = datas[i];
                        ds.removeLocal(ds.findById(data['__id']), true);
                    }
        ...
        for (var k in this.fields) {
            ...
            if (field.type == 'dataset') {
                ...
                if (r && r.data[field.name]) {
                    var record = new $A.Record(r.data, r.data[field.name]);
...
// if(valid !== false) if(!this.validCurrent())return;
        ...
        if (index <= 0 || (this.autocount && (index > this.total + this.getNewRecords().length))) {
            return;
        }
...
// }
            ...
            if (!this.isValid) {
                ...
                for (var key in valid) {
                    ...
                    if (unvalidRecord.ds.fields && unvalidRecord.ds.fields[key] && unvalidRecord.ds.fields[key].pro) {
                        if (unvalidRecord.ds.fields[key].pro.prompt) {
                ...
                $A.showInfoMessage(_lang['dataset.info'], unvalidMessage || _lang['dataset.info.validate'], null, 350, 85 + errorCount * 20);
            }
        }
        return this.isValid;
    },
    ...
    doQuery: function (page, opts) {
        ...
        var parse_para = Object.assign(para, q)
        if (sf.qtId) Ext.Ajax.abort(sf.qtId);
        sf.qtId = Ext.Ajax.request({
            url: qurl,
            method: 'POST',
            success: function (res, opts) {
                if (!Ext.decode(res.responseText).success) {
                    var errorMsg = Ext.decode(res.responseText).message;
                    $A.showErrorMessage(_lang['window.error'], errorMsg + '</br>', null, 400, 150);
                    this.loading = false;
                    this.keepData = false;
                    this.fireEvent('loadsuccess', this);
                    $A.Masker.unmask(Ext.getBody());
                } else {
                    sf.onLoadSuccess(res, opts);
                }
            },
            ...
            Ext.iterate(d, function (k, item) {
                if (fields && fields.indexOf(k) == -1) {
                    ...
                    if (item && item.xtype == 'dataset') {
                        ...
                        var ds = new $A.DataSet({});// $au(item.id);
...
// this.fireEvent(event,this);
    ...
    commitRecords: function (datas, fire, record) {
        ...
        for (var i = 0, l = datas.length; i < l; i++) {
            var data = datas[i];
            var r = this.findById(data['__id']);
            if (!r) continue;
            if (r.isNew) this.total++;
            r.commit();
            var haschange = false;
            for (var k in data) {
                var field = k;
                ...
                if (f && f.type == 'dataset') {
                    ...
                    // 2019-02-21 guiyuting 添加data[k]为空的判断
                    if (data[k] && data[k].record) {
                    ...
                    var ov = r.get(field);
                    var nv = data[k]
                    if (field == '__id' || field == '__status' || field == '__parameter_parsed__') continue;
                    if (f) {
...
// res.error.message||res.error.stackTrace,null,400,200);
    ...
    onLoadSuccess: function (res, options) {
        try {
            ...
            if (!Ext.decode(res.responseText).rows) {
                Ext.decode(res.responseText).rows = []
            }
            var records = [].concat(Ext.decode(res.responseText).rows);
            // var total = res.result.totalCount;
            var total = Ext.decode(res.responseText).total;
            var datas = [];
            if (records.length > 0) {
                for (var i = 0, l = records.length; i < l; i++) {
                    var item = {
...
$A.Record.Field.prototype = {
     ...
     *
     */
    setLovCode: function (m) {
        this.setPropertity("lovcode", m)
    },
    /**
...
$A.Component = Ext.extend(Ext.util.Observable, {
    ...
    initComponent: function (config) {
        ...
        // if (!Ext.isIE8) {
        //     $('div.item-comboBox-view').niceScroll({horizrailenabled: false});
        // }
    },
    processListener: function (ou) {
...
//    		debugger;
//    	}
        if ((this.clientresize && (!Ext.isEmpty(this.marginwidth) || !Ext.isEmpty(this.marginheight)) || this.constructor == $A.Window)) {
// this.windowResizeListener();//TODO:以后修改服务端component,去掉自身尺寸的判断
            ...
            $A.auWindow.on("resize", this.windowResizeListener, this);
        }
    },
...
// Ext.getBody().setStyle('overflow','hidden')
        ...
        if (!Ext.isEmpty(this.marginheight) && !this.autoadjustheight) {
            ht = Aurora.getViewportHeight() - Math.max($A.usedHeight, 0);
        ...
        if (!Ext.isEmpty(this.marginwidth)) {
            ...
            var v = wd - this.marginwidth - Math.max($A.usedWidth, 0);
            this.setWidth(v);
            // 非标准做法,中集特殊要求！
...
// [ou]("mouseout", sf.onMouseOut, sf);
    ...
    onTipMouseOver: function () {
        ...
        $A.ToolFloat.show(tipIcon, sf.promptContentRenderer());
    },
...
// sf.shadow.show();
            ...
            // $(".item-comboBox-view").niceScroll({horizrailenabled: false});
        },
        trigger: function (e) {
            var sf = this;
...
// sf.shadow.hide();
        ...
        correctViewSize: function () {
            ...
            sf.setHeight(Math.max(Math.min(table.getHeight() + 5, sf.maxHeight), 20));
            sf.setWidth(width);
...
$A.Button.getTemplate = function (id, text, width, height) {
...
// Aurora.Button({"id":"'+id+'"});</script>';
    return '<TABLE class="item-btn " id="' + id + '" style="WIDTH: ' + (width || 80) + 'px; HEIGHT: ' + (height || 26) + 'px" cellSpacing="0"><TBODY><!--<TR><TD class="item-btn-tl"><I></I></TD><TD class="item-btn-tc"></TD><TD class="item-btn-tr"><I></I></TD></TR>--><TR><!--<TD class="item-btn-ml"><I></I></TD>--><TD class="item-btn-mc" auskin="white"><BUTTON hideFocus skin="white" style="HEIGHT: 17px" atype="btn"><div auskin="white">' + text + '</div></BUTTON></TD><!--<TD class="item-btn-mr"><I></I></TD>--></TR><!--<TR><TD class="item-btn-bl"><I></I></TD><TD class="item-btn-bc"></TD><TD class="item-btn-br"><I></I></TD></TR>--></TBODY></TABLE><script>new Aurora.Button({"id":"' + id + '"});</script>';
}
$A.SwitchButton = Ext.extend($A.Component, {
    ...
    selectButton: function (value) {
        ...
        var oli = this.wrap.child('li[code=' + this.activeValue + ']'), li = this.wrap.child('li[code=' + value + ']'),
            ov;
        if (li) {
            if (oli) {
...
// },
    onBlur: function (e) {
        var sf = this;
        if(!sf.readonly && sf.value){
            var v = sf.value;
            var rv = $A.parseScientific(sf.fixPrecision(sf.parseValue(v)));
            sf.value = rv;
        }
        $A.ToolTip.hide();
...
//			y=((_y+PH+H)>BH?((_y-H)<0?(_y+PH):(_y-H)):(_y+PH))+st+2;
        ...
        // if (!Ext.isIE8) {
        //     setTimeout(function() {
        //         $('div.item-comboBox-view').getNiceScroll().resize();
        //     },100)
        // }
...
// this.doQuery();
    ...
    setOptions: function (name) {
        var ds = typeof(name) === 'string' ? $au(name) : name;
...
$A.DateField = Ext.extend($A.Component, {
    bodyTpl: ['<TABLE cellspacing="0">',
        ...
        '</TBODY>',
        '</TABLE>'],
    preMonthTpl: '<DIV class="item-dateField-pre" title="{preMonth}" onclick="$au(\'{id}\').preMonth()"></DIV>',
    nextMonthTpl: '<DIV class="item-dateField-next" title="{nextMonth}" onclick="$au(\'{id}\').nextMonth()"></DIV>',
    preYearTpl: '<DIV class="item-dateField-preYear" title="{preYear}" onclick="$au(\'{id}\').preYear()"></DIV>',
    nextYearTpl: '<DIV class="item-dateField-nextYear" title="{nextYear}" onclick="$au(\'{id}\').nextYear()"></DIV>',
    ...
    draw: function (date) {
        // 用来保存日期列表
        var arr = [], year = date.getFullYear(), month = date.getMonth() + 1, hour = date.getHours(),
    ...
    initView: function (num, width, isYear) {
        ...
        html.push("");
        if (isYear) {
            html.push("<tr><td><div class='item-dateField-pre' onclick='$au(\"" + this.id + "\").initView(" + (num - 10) + "," + width + ",true)'></div></td>");
            html.push("<td><div class='item-dateField-close' onclick='$au(\"" + this.id + "\").onViewHide()'></div></td>")
            html.push("<td><div class='item-dateField-next' onclick='$au(\"" + this.id + "\").initView(" + (num + 10) + "," + width + ",true)'></div></td></tr>");
        } else {
            html.push("<td colspan='3' align='center'><div class='item-dateField-close' onclick='$au(\"" + this.id + "\").onViewHide()'></div></td>")
        }
        html.push("</table>");
...
// this.processListener('on');
    ...
    initDateField: function () {
        this.popup.setStyle({'width': 200 * this.viewsize + 'px'})
 ...
 *
 */
$A.Calendar = Ext.extend($A.Component, {
    bodyTpl: ['<TABLE cellspacing="0" class="calendar-table" style="width:100%;border:#d4d4d4 1px solid;">',
        '<CAPTION class="item-calendar-caption" style="width:100%;height:35px;">',
        '{preMonthBtn}',
        '{nextMonthBtn}',
        '<DIV class="today" style="float:left; width:60px;height:28px;background:#f7f8f7;border:1px solid #d4d4d4;line-height:28px;text-align:center;">今天</DIV>',
        '<DIV class="item-calendar-next" title="{nextMonth}"></DIV>',
        '<SPAN style="float:right;line-height:30px;font-size:16px;margin-left:10px;margin-right:10px;">',
        '<SPAN atype="item-year-span" style="margin-right:5px;cursor:pointer"></SPAN>',
        '<SPAN atype="item-month-span" style="cursor:pointer"></SPAN>',
        '</SPAN>',
        '<DIV class="item-calendar-pre" title="{preMonth}"></DIV>',
        '</CAPTION>',
        '<THEAD class="item-calendar-head">',
        '<TR>',
        '<TD>{sun}</TD>',
        '<TD>{mon}</TD>',
        '<TD>{tues}</TD>',
        ...
        '</TBODY>',
        '</TABLE>'],
    preMonthTpl: '<DIV class="item-calendar-pre" title="{preMonth}" onclick="$au(\'{id}\').preMonth()"></DIV>',
    nextMonthTpl: '<DIV class="item-calendar-next" title="{nextMonth}" onclick="$au(\'{id}\').nextMonth()"></DIV>',
    constructor: function (config) {
        $A.Calendar.superclass.constructor.call(this, config);
    },
    initComponent: function (config) {
        $A.Calendar.superclass.initComponent.call(this, config);
        if (this.height) this.rowHeight = (this.height - 18 * (Ext.isIE ? 3 : 2)) / 6;
        var data = {
            sun: _lang['datefield.sun'],
            mon: _lang['datefield.mon'],
...
//			data.nextMonthBtn = new Ext.Template(this.nextMonthTpl).apply({nextMonth:_lang['datefield.nextMonth'],id:this.id});
        ...
        this.yearSpan = this.body.child("span[atype=item-year-span]");
        this.monthSpan = this.body.child("span[atype=item-month-span]");
        this.monthPre = this.body.child("div.item-calendar-pre");
        this.monthNext = this.body.child("div.item-calendar-next");
        this.today = this.body.child("div.today");
        var calendarTable = this.wrap.child("table.calendar-table");
        var ch = Aurora.getViewportHeight();
        var cth = ch - this.marginheight;
        calendarTable.setHeight(cth);
        this.predraw();
        // this.popup = new
        // Ext.Template(this.popupTpl).append(this.wrap.dom,true);
    },
    processListener: function (ou) {
        $A.Calendar.superclass.processListener.call(this, ou);
        this.body[ou]("click", this.onSelect, this);
//		this.yearSpan[ou]("click",this.onViewShow,this);
//		this.monthSpan[ou]("click",this.onViewShow,this);
        this.monthPre[ou]("click", this.preMonth, this);
        this.monthNext[ou]("click", this.nextMonth, this);
        this.today[ou]("click", this.nowMonth, this);
    },
    initEvents: function () {
        $A.Calendar.superclass.initEvents.call(this);
        this.addEvents(
            /**
             * @event select 选择事件.
             * @param {Aurora.DateField}
    ...
    destroy: function () {
        $A.Calendar.superclass.destroy.call(this);
        delete this.preMonthBtn;
        delete this.nextMonthBtn;
        delete this.body;
    },
    ...
    onSelect: function (e, t) {
        var sf = this, td = Ext.get(t), svc = sf.lovservice, ctx = sf.context, url;
        if (td.hasClass("item-day") && !td.hasClass("item-day-besides")) {
            var _date = td.getAttribute("_date");
            url = ctx + 'sys_calendar.screen?url=' + encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/' + svc + '/query')) + '&service=' + svc + '&date=' + _date;
            sf.win = new $A.Window({
                title: sf.title || 'calendar',
                fullScreen: true,
                url: Ext.urlAppend(url, "lovid=" + sf.id + "&lovautoquery=" + (Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery))
            });
...
//	},
    ...
    predraw: function (date, notFire) {
        ...
        this.year = date.getFullYear();
        this.month = date.getMonth() + 1;
        this.draw(new Date(this.year, this.month - 1, 1));
        if (!notFire) this.fireEvent("draw", this);
    },
    /**
     * 渲染日历
    ...
    draw: function (date) {
        ...
        for (var i = 1; i <= monthDay; i++) {    //1号到这个月最后1天
...
//			if(this.rowHeight)row.setHeight(this.rowHeight);
            ...
            for (var i = 1; i <= 7; i++) {
                ...
                if (Ext.isDefined(d)) {
                    ...
                    if (d) {
                        cell.set({'c_index': i - 1});
                        cell.addClass(oldMonth == d.getMonth() ? "item-day" : "item-day item-day-besides");
                        /*
						 * if(((i-1)==0||(i-1)==6)&&(date.getMonth()==d.getMonth())){
						 * cell.set({'style':'color:#f15353'}); }
						 */
                        ...
                        var spanWrap = Ext.get(document.createElement("span"));
                        var contentWrap = Ext.get(document.createElement("div"));
                        contentWrap.addClass("content-wrap");
                        contentWrap.update(this.contentRenderer());
                        spanWrap.addClass("day");
                        var span = Ext.get(document.createElement("span"));
                        span.update(this.renderCell(cell, d, d.getDate(), month) || d.getDate());
                        if ((this.renderCell(cell, d, d.getDate(), month) || d.getDate() == nowDay) && d.getFullYear() == nowYear && d.getMonth() == nowMonth) {
                            span.addClass("nowDate");
                        }
                        ...
                        if (this.isSame(d, new Date())) cell.addClass("onToday");
                        // 判断是否选择日期
//						if(this.isSame(d, new Date())) this.onSelectDay(cell);
                    } else cell.update('&#160;');
                }
            }
        }
...
// sf.pageSizeInfo.update(_lang['toolbar.pageSize']);
    ...
    createAnchor: function (text, page) {
        return '<a href="javascript:$au(\'' + this.dataSet.id + '\').goPage(' + page + ')">' + text + '</a>';
    },
...
// A.focusWindow = null;
        ...
        initComponent: function (config) {
            ...
            sf.titleheight = (sf.titleheight || 25);
            if (sf.fullScreen) {
                sf.width = A.getViewportWidth() - 2;
                sf.height = A.getViewportHeight() - sf.titleheight - 2;
                sf.draggable = false;
//            sf.marginheight=1;
//            sf.marginwidth=1;
            } else if (sf.side) {
                ...
                if (sf.side == 'top' || sf.side == 'bottom') {
                    if (sf.height < 1) {
                        var h = A.getViewportHeight() - 2;
                        sf.height = parseInt(h * sf.height);
                    }
                    sf.width = A.getViewportWidth() - 2;
                } else if (sf.side == 'left' || sf.side == 'right') {
                    if (sf.width < 1) {
//                sf.height=A.getViewportHeight()-42;
                        var w = A.getViewportWidth() - 2;
                        sf.width = parseInt(w * sf.width);
                    }
                    sf.height = A.getViewportHeight() - sf.titleheight - 2;
                }
            } else {
                ...
                var w = A.getViewportWidth() - 2;
                if (sf.width < 1) {
                    sf.width = parseInt(sf.width * w);
                } else {
                    sf.width = sf.width;
                }
                if (sf.height < 1) {
                    sf.height = parseInt(sf.height * h);
                } else {
                    sf.height = sf.height;
                }
            }
            ...
            if (sf.side) {
                if (sf.side == 'top') {
                    ...
                    var scroll = sf.wrap.parent().getScroll(),
                        sy = scroll.top;
                    sf.move(A.getViewportWidth() - 3 - sf.width, sy);
                    if (Ext.isIE8) {
        ...
        getTemplate: function (config) {
            if (config.isDialog) {
                return [
                    ...
                    '<TABLE cellSpacing="0" class="win-cap" unselectable="on"  onselectstart="return false;" style="-moz-user-select:none;"  cellPadding="0" width="100%" border="0" unselectable="on">',
                    '<TBODY>',
                    '<TR>',
                    ...
                    '<DIV class="win-content win-dialog-content" id="win-content" atype="window.body" style="position:relative;width:{bodywidth}px;" unselectable="on"></DIV>',
                    '</TD>',
                    '</TR>',
                    '</TBODY>',
                    '</TABLE>',
        ...
        close: function (nocheck) {
        	 ...
        	 */
                ...
                if (sf.modal) {
                    ...
                    if (cw) {
                        ...
                        if (cover) {
                            ...
                            setTimeout(function () {
                                cover.removeClass('animated fadeOut03');
                            }, 600);
                        }
                    }
                }
                ...
                var d = new Ext.util.DelayedTask(function () {
                    ...
                    //     $("html").niceScroll({enablemousewheel: true});
                    // }
...
// }
    ...
    A.showTypeMessage = function (title, msg, width, height, css, callback) {
        var msg = '<div class="win-icon ' + css + '"><div class="win-type" style="max-height:' + (height - 55) + 'px;">' + msg + '</div></div>';
...
// }
    ...
    A.showOkCancelWindow = function (title, msg, okfun, cancelfun, width, height) {
        ...
        if (!Ext.isEmpty(msg, true)) {
            msg = '<div class="win-icon win-question"><div class="win-type" style="width:' + (width - 70) + 'px;max-height:' + (height - 55) + 'px;">' + msg + '</div></div>';
            cmp.body.update(msg + '<center style="float:right;padding:5px;"><table cellspacing="5"><tr><td style="padding-left:5px;padding-right:5px;">' + okbtnhtml + '</td><td style="padding-left:5px;padding-right:5px;">' + cancelbtnhtml + '</td></tr></table></center>', true, function () {
                var okbtn = $au(okid);
                var cancelbtn = $au(cancelid);
                cmp.cmps[okid] = okbtn;
    ...
    A.showYesNoCancelWindow = function (title, msg, yesfun, nofun, width, height) {
        ...
        var id = Ext.id(),
            ...
            cancelbtnhtml = A.Button.getTemplate(cancelid, _lang['window.button.cancel']),
            cmp = new A.Window({
                id: 'aurora-msg-yes-no-cancel' + id,
                closeable: true,
                title: title,
                height: height || 120,
                width: width || 300
        ...
        if (!Ext.isEmpty(msg, true)) {
            msg = '<div class="win-icon win-question"><div class="win-type" style="width:' + (width - 70) + 'px;max-height:' + (height - 55) + 'px;">' + msg + '</div></div>';
            cmp.body.update(msg + '<center><table cellspacing="5"><tr><td style="padding-left:5px;padding-right:5px;">' + yesbtnhtml + '</td><td style="padding-left:5px;padding-right:5px;">' + nobtnhtml + '</td><td>' + cancelbtnhtml + '</td></tr></table></center>', true, function () {
                var yesbtn = $au(yesid),
                    nobtn = $au(noid),
                    cancelbtn = $au(cancelid);
    ...
    A.showOkWindow = function (title, msg, width, height, callback) {
        ...
        if (!Ext.isEmpty(msg, true)) {
            cmp.body.update(msg + '<center style="padding:12px;float:right;">' + btnhtml + '</center>', true, function () {
                var btn = $au(yesid);
                cmp.cmps[yesid] = btn;
    ...
    A.showUploadWindow = function (path, title, source_type, pkvalue, max_size, file_type, callback) {
        new Aurora.Window({
            id: 'upload_window',
            url: path + '/upload.screen?callback=' + callback + '&pkvalue=' + pkvalue + '&source_type=' + source_type + '&max_size=' + (max_size || 0) + '&file_type=' + (file_type || '*.*'),
...
(function (A) {
    ...
    A.MultiTextField = Ext.extend(A.TextField, {
        infoTpl: ['<div class="item-receiver-info" _data="{data}" style="float:left;margin-right:10px;position:relative;"><span class="item-receiver-info-name">{text};</span><span class="item-receiver-info-close"></span></div>'],
        processListener: function (ou) {
            var sf = this;
        ...
        removeItem: function (t) {
            if (t) {
                ...
                if (sf.isgrid == false) {
                    var item_multiLov = sf.wrap.child(".item-multiLov");
                    if (item_multiLov.dom.childElementCount === 1) {
                        sf.el.setHeight(28);
                        sf.el.setStyle("display", "block");
                    }
                }
            }
        },
            ...
            if (sf.fetchremote) {
                ...
                sf.qtId = A.request({
                    url: url, para: p, success: function (res) {
                        var r = new A.Record({});
                        if (res.rows) {
                            var datas = [].concat(res.rows), l = datas.length;
                            if (l > 0) {
...
(function (A) {
    ...
    A.Lov = Ext.extend(A.TextField, {
        ...
        initComponent: function (config) {
            ...
            var lovCode = sf.lovcode = config.lovcode;
            if(lovCode){
                var lovCodeArray = lovCode.split('?');
                if(lovCodeArray.length==2){
                    var lovParaArray = lovCodeArray[1].split('&');
                    for(var i = 0;i<lovParaArray.length;i++){
                        var paras = lovParaArray[i].split('=');
                        if(paras.length == 2){
                            sf.para[paras[0]] = paras[1];
                        }
                    }
                }
            }
...
// },
        ...
        fetchRecord: function () {
            ...
            if (!Ext.isEmpty(lovCode)) {
                var lovCodeArray = lovCode.split('?');
                url = Ext.urlAppend(sf.context + 'common/lov/' + lovCodeArray[0], Ext.urlEncode(sf.getLovPara()));
            }
            if (record == null && binder)
                record = binder.ds.create({}, false);
            record.isReady = false;
            ...
            A.slideBarEnable = sidebar.enable;
            sidebar.enable = false;
            if (Ext.isEmpty(v) || Ext.isEmpty(lovCode)) {
                sf.fetching = false;
                record.isReady = true;
                sidebar.enable = A.slideBarEnable;
                return;
...
// sf.setRawValue(_lang['lov.query'])
            ...
            url = Ext.urlAppend(url,Ext.urlEncode(p));
            sf.qtId = A.request({
                url: url, para: p, success: function (res) {
                    var r = new A.Record({});
                    if (res.rows) {
...
// sf.setRawValue(_N);
        ...
        showLovWindow: function () {
            ...
            var v = sf.getRawValue(),
                lovurl = sf.lovurl,
                cw = Aurora.getViewportWidth(),
                ch = Aurora.getViewportHeight(),
                lovcode = sf.lovcode, ctx = sf.context,
                lgh = sf.lovgridheight,
                url;
            if (sf.lovwidth) {
                if (sf.lovwidth < 1) {
                    var w = parseInt(sf.lovwidth * cw);
                } else {
                    var w = sf.lovwidth;
                }
            } else {
            ...
            } else if (!Ext.isEmpty(lovcode)) {
// url = sf.context + 'sys_lov.screen?url='+encodeURIComponent(sf.context +
// 'sys_lov.svc?svc='+sf.lovservice + '&'+
...
//                 url = ctx + 'sys_lov.screen?url=' + encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/' + svc + '/query', Ext.urlEncode(sf.getLovPara()))) + '&service=' + svc;
                var lovCodeArray = lovcode.split('?');
                url = ctx + 'common/auroraLov/' + lovCodeArray[0] + '?lov_param=' + encodeURIComponent(Ext.urlEncode(sf.getLovPara()));
            ...
            if (url) {
                ...
                sf.win = new A.Window({
                    title: sf.title || 'Lov',
                    url: Ext.urlAppend(url, "lovid=" + sf.id + "&key=" + encodeURIComponent(v) + "&gridheight=" + Math.max(lgh || 355, 355) + "&innerwidth=" + (w - 15) + "&lovautoquery=" + (Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery) + "&lovlabelwidth=" + Math.max(sf.lovlabelwidth || 110, 110) + "&lovpagesize=" + (sf.lovpagesize || '')),
                    height: h,
                    width: w
                });
                sf.win.on('close', sf.onWinClose, sf);
     ...
     *          <p>
     *          MultiLov 值列表组件.
     * @author ping.xiang@hand-china.com
     * @constructor
     * @param {Object}
     *            config 配置对象.
     */
    A.MultiLov = Ext.extend(A.MultiTextField,{
        ...
        showLovWindow : function(){
            ...
            sf.saveRecord ={};
            var item_multiLov = sf.wrap.child(".item-multiLov");
            var receivers = item_multiLov.dom.children;
            var mapss = sf.getMapping();
            if(sf.record.get(mapss[0].to)!= ""){
                sf.saveRecord[0] = sf.record.get(mapss[0].to);
                sf.saveRecord[1] = sf.record.get(mapss[1].to);
            }
            ...
            if(sf.lovheight){
                if(sf.lovheight < 1){
                    var vh = Aurora.getViewportHeight();
                    var h = parseInt(sf.lovheight * vh);
                }else{
                    var h = sf.lovheight;
                }
            }else{
...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
                ...
                fieldRecord = encodeURIComponent(JSON.stringify(fieldRecord),"utf-8");
                if(showCode){
                    var mapps = sf.getMapping();
                    var showId = mapps[0].from;
                    url = ctx + 'common/auroraMultiLov/' + lovCodeArray[0] + '?lov_param='+encodeURIComponent(Ext.urlAppend(Ext.urlEncode(sf.getLovPara())))+'&fieldRecord='+fieldRecord+'&showCode='+showCode+'&showId='+showId;
                }else{
                    var mapps = sf.getMapping();
                    var showId = mapps[0].from;
                    var showCode = mapps[1].from;
                    url = ctx + 'common/auroraMultiLov/' + lovCodeArray[0] + '?lov_param='+encodeURIComponent(Ext.urlAppend(Ext.urlEncode(sf.getLovPara())))+'&fieldRecord='+JSON.stringify(fieldRecord)+'&showCode='+showCode+'&showId='+showId;
                }

            }
            if(url) {
                sf.isWinOpen = true;
                sf.win = new A.Window({title:sf.title||'multiLov', url:Ext.urlAppend(url,"lovid="+sf.id+"&key="+encodeURIComponent(v)+"&gridheight="+(sf.lovgridheight-60||280)+"&innerwidth="+(w-30)+"&innergridwidth="+Math.round((w-90)/2)+"&lovautoquery="+(Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery)+"&lovlabelwidth="+(sf.lovlabelwidth||75)+"&lovpagesize="+(sf.lovpagesize||'')), height:Math.max(h||450,450),width:w});
                sf.win.on('close',sf.onWinClose,sf);
            }
        },
//    clearBind : function(){
//    	var sf = this;
//    	$A.MultiLov.superclass.clearBind.call(sf);
//    	sf.saveRecord ={};
//    	var item_multiLov = sf.wrap.child(".item-multiLov");
//    	var receivers = item_multiLov.dom.children;
//		for(var i=0;i<receivers.length;i++){
//    		if(Ext.get(receivers[i]).hasClass("item-receiver-info")){
//    			sf.saveRecord[i] = receivers[i].getAttribute("_data");
//    		}
//    	}
//    },
        reset : function(r){
            var sf = this;
            sf.setValue(Ext.isEmpty(sf.originalValue)?sf.originalValue:'');
            sf.clearInvalid();
            sf.applyEmptyText();
            if(r.length == 0){
                sf.clearAllItems();
     ...
     */
    A.Tledit = Ext.extend($A.TextField, {
        constructor: function (config) {
            var sf = this;
            sf.isWinOpen = false;
        ...
        processValue: function (v) {
            var sf = this,
                ...
                //20190109 jalyn.xing 修改获取语言环境的方式
                currentLang = _locale;
            sf.record.set(fieldName + '_' + currentLang + '_mutl', v);
            return $A.Tledit.superclass.processValue.call(sf, v);
        },
        onWrapFocus: function (e, t) {
            var sf = this;
            e.stopEvent();
        ...
        commit: function (r, currentLang) {
            ...
            if (record && r && currentLang) {
                ...
                if (record.get('__tls')) {
                    record.get('__tls')[fieldName] = r.data;
                } else {
                    var tlFieldValue = {};
                    tlFieldValue[fieldName] = r.data;
                    record.set('__tls', tlFieldValue);
                }
            }
            if (sf.win) sf.win.close();
        },
        ...
        showTleditWindow: function () {
            ...
            var sf = this,
                ...
                record = sf.record,
                dto = sf.dto,
                id = record.get(sf.dtoid) ? record.get(sf.dtoid) : '',
                width = sf.tleditWidth || 400,
                height = sf.tleditHeight || 190,
                margin = sf.tleditMargin || 30,
                fieldName = sf.getBindName(),
                url = ctx + 'sys/auroraMultiLanguage?id=' + id + '&dto=' + dto + '&field=' + encodeURIComponent(fieldName ? fieldName : '');

            sf.isWinOpen = true;
            sf.win = new A.Window({
                title: sf.title || '输入多语言信息',
                url: Ext.urlAppend(url, "tleditid=" + sf.id),
    ...
    /**
     * @class Aurora.ImagePicker
     * @extends Aurora.TextField
     *          <p>
     *          imagePicker 图片选择器组件
     * @author zhao.xu@hand-china.com
     */
    A.ImagePicker = Ext.extend($A.TextField, {
        constructor: function (config) {
            var sf = this;
            sf.isWinOpen = false;
            sf.fetching = false;
            sf.fetchremote = true;
            sf.maxHeight = 240;
            A.ImagePicker.superclass.constructor.call(this, config);
            this.images = config.images;
        },
        initComponent: function (config) {
            A.ImagePicker.superclass.initComponent.call(this, config);
            var sf = this;
            sf.trigger = sf.wrap.child('div[atype=triggerfield.trigger]');
            sf.imgWrap = sf.wrap.child('div.item-image-box');
            sf.el = sf.wrap.child('input[atype=field.input]');
            elH = sf.el.getHeight();
        },
        processListener: function (ou) {
            A.ImagePicker.superclass.processListener.call(this, ou);
            var sf = this;
            sf.trigger[ou]("click", sf.onTriggerClick, sf);
        ...
        clearValue: function () {
            var sf = this;
            var oldImg = sf.imgWrap.child(".img_item");
            if (oldImg) {
                oldImg.remove();
            }
            this.setValue('', true);
        ...
        setValue: function (v, silent) {
            ...
            if (v) {
                ...
                img = '<span class="img_item" style="position:relative;width:30px;height:30px;display:block;"><img style="width:30px;height:30px;" src="' + cxt + v + '" /><span class="img_close"></span></span>';
                sf.imgWrap.update(img);
                sf.el.setHeight(0);
//    		imgBox.update(img.join(''));
                sf.close = sf.imgWrap.child(".img_close");
                sf.close.on("click", sf.onCloseImg, sf);
            }
            $A.Field.superclass.setValue.call(sf, v, silent);
        },
        onTriggerClick: function (e) {
...
//		}
        ...
        showIamgeWindow: function () {
            if (!this.popupWrap) {
                ...
                this.popupWrap = new A.Window({title: this.title || '图片库', width: w, height: h});
                this.popupContent = this.popupWrap.wrap.child(".win-content");
                this.view = this.popupContent.update('<ul class="image-wrap"></ul>').child('ul')
                    .on('dblclick', this.onViewDblClick, this)
            }
            if (this.images) {
                this.initList();
            }
            // $(".win-content").niceScroll();
        },
        onIamgeWindowClose: function (pw) {
            var sf = this;
            sf.isWinOpen = false;
            this.popupWrap.close();
            if (!Ext.isIE6 && !Ext.isIE7) {// TODO:不知什么地方会导致冲突,ie6 ie7 会死掉
                sf.focus();
            } else {
                (function () {
...
//    	this.el.setStyle("display","none");
        ...
        onViewDblClick: function (e, t) {
            if (t.tagName != 'IMG') {
                return;
            }
            this.onSelect(t);
            if (this.popupWrap) {
                this.onIamgeWindowClose(this.popupWrap);
            }
        },
        onSelect: function (t) {
            var target = Ext.get(t).parent("li");
            var index = target.dom.tabIndex;
            var cxt = this.context;
            if (index == -1) return;
            var sf = this;
            if (sf.blankoption) {
            ...
            if (index != -1) {
                var src = Ext.get(t).getAttribute("data-src");
                img = '<span class="img_item" style="position:relative;width:30px;height:30px;display:block;"><img style="width:30px;height:30px;" src="' + cxt + src + '" /><span class="img_close"></span></span>';
//			img.src = cxt + src;
//			img.width = 30;
//			img.height = 30;
//			img.push('<img style="width:30px;height:30px;" src="',cxt + src,'" />');
                sf.imgWrap.update(img);
                sf.el.setHeight(0);
//			imgBox.update(img.join(''));
                sf.close = sf.imgWrap.child(".img_close");
            ...
            A.ImagePicker.superclass.setValue.call(this, src, null);
        },
        onCloseImg: function (e) {
            var closeImg = Ext.get(e.target);
            closeImg.parent(".img_item").remove();
            var src = '';
            this.el.setHeight(elH);
            A.ImagePicker.superclass.setValue.call(this, src, null);
        },
        initList: function () {
            var v = this.view,
...
//				n=1;
//			}
            Ext.each(this.images, function (d, i) {
                var n = imgPath + '/' + d;
                sb.push('<li');
                sb.push(' tabIndex="', i + n, '"><img style="display:inline-block;" width="', imgSize, '" height="', imgSize, '" data-src="', n, '" src="', cxt + n, '" /></li>');
            }, this);
            sb.push('<div class="clearfix" />');
            v.update(sb.join(''));
            // $(".item-images-view").niceScroll();
//		}
        }
    });
 ...
 * 'aurora-item-popup',popup = A.CmpManager.get(id); if(popup)return popup;
...
$A.TextArea = Ext.extend($A.Field, {
    ...
    onKeyUp: function (e) {
        if ((e.keyCode > 65 && e.keyCode < 89 || e.keyCode > 65 && e.keyCode < 110)) {
            !this.readonly && this.setValue(this.processValue(this.processMaxLength(this.getRawValue())));
        }
    },
    onKeyDown: function (e) {
    }
...
$A.Customization = Ext.extend(Ext.util.Observable, {
    ...
    onClick: function () {
        ...
        new Aurora.Window({
            ...
            url: context_path + 'modules/sys/sys_customization_window.screen?screen_path=' + screen_path + '&id=' + this.cmp.id,
            title: '个性化设置',
            height: 170,
            width: 400
        });
...
$A.MultiComboBox = Ext.extend($A.ComboBox, {
    ...
    setOptions: function (name) {
        var sf = this,
            ds = typeof(name) === 'string' ? $au(name) : name;
...
$A.Form = Ext.extend($A.Component, {
    constructor: function (config) {
        var sf = this;
        sf.toggleBtnId = config.togglebtnid;
        sf.defaultExpand = config.defaultexpand;
        sf.currentExpand = config.defaultexpand;
        $A.Form.superclass.constructor.call(this, config);
    },
    initComponent: function (config) {
        $A.Form.superclass.initComponent.call(this, config);
        var sf = this;
        /*
    ...
    processListener: function (ou) {
        var sf = this;
        $A.Form.superclass.processListener.call(sf, ou);
        if (sf.shrink) {
...
//    onMouseover : function(e){
//    	var text = this.title;
//    	$A.ToolTip.show(this.formTitle,text);
//    },
    /*
     * 添加一个boolean的参数force，
     * force值:
...
$A.ButtonGroup = Ext.extend($A.Component, {
    initComponent: function (config) {
        $A.ButtonGroup.superclass.initComponent.call(this, config);
        var sf = this,
...
//			if(!sf.isopen)sf.body.hide();
    ...
    processListener: function (ou) {
        var sf = this;
        $A.ButtonGroup.superclass.processListener.call(sf, ou);
...
$A.TagBox = Ext.extend($A.Component, {
    constructor: function (config) {
        var sf = this;
        sf.bindTarget = config.bindtarget;
        sf.ds = $au(sf.bindTarget);
        $A.TagBox.superclass.constructor.call(sf, config);
    },
    initComponent: function (config) {
        $A.TagBox.superclass.initComponent.call(this, config);
        var sf = this;
        sf.processDataSetListener('on');
    },
    processListener: function (ou) {
        var sf = this;
        $A.TagBox.superclass.processListener.call(sf, ou);
