...
function deleteFileRecord(did,id) {
    ...
    Aurora.showConfirm(_lang['window.button.ok'],_lang['upload.delete.sure'],function(win){
        ...
        $au(upload).fireEvent("delete", this);
    })
}

function upFileRecord(did,id) {
    var ds = $au(did),record = ds.findById(id),i = ds.data.indexOf(record);    
    if(i!=-1&& i!=0){
	...
	if (record) {
        if($A.uploadcmps && $A.uploadcmps.length >0){
            Ext.each($A.uploadcmps,function(uploader){
                var xhr = uploader.map[record.get('fileId')];
                if(xhr){
                    try{
                    xhr.abort();
                    }catch(e){}
            ...
            window[upid].cancelUpload(record.get('fileId'));
        }
...
function processPercent(record,canDelete,reorder) {
	...
	if (Ext.isEmpty(percent) || status == 1) {
		html += '<div style="float:left;margin-left:10px;color:#808080">'
				+ formatFileSize(record.get('fileSize'))
                + '</div>'
		...
		html += '<div style="float:left;margin-left:10px;color:#808080">'
				+ formatFileSize(record.get('fileSize')) + '</div>';
...
function atmRenderer(value, record, name, canDelete, deleteControl,reorder) {
	...
	var html = '<div class="' + c + '">' + a + '<div style="float:left">' + (percent == -1 ? value :'<a target="_self" '+(atmTypeRef(record.get("fileName")) ? 'ref="img"' : '') +' href="'+window[upid+'_download_path']+'?attachmentId='+record.get('attachmentId')+'&accessId='+record.get('accessId')+'&tableName='+record.get('tableName')+'&tablePkValue='+record.get('tablePkValue')+'\">'
...
$A.UploadList = Ext.extend($A.Component,{
    ...
    onUpdate : function(ds,record, name, value){
        if(name=='percent' || name == 'status') {
            ...
            if (Ext.isEmpty(percent) || status == 1) {
                ...
                this.wrap.child("#atm_"+record.id).child('.j').update('<a target="_self" '+(atmTypeRef(record.get("fileName")) ? 'ref="img"' : '') +' href="'+this.downloadurl+'?attachmentId='+record.get('attachmentId')+'&accessId='+record.get('accessId')+'&tableName='+record.get('tableName')+'&tablePkValue='+record.get('tablePkValue')+'\" title="'+record.get("fileName")+'">'+record.get('fileName')+'</a>');
                this.wrap.child("#atm_"+record.id).child('.k').update(formatFileSize(record.get('fileSize'))+'<span class="d">'+Aurora.formatDateTime(record.get('creationDate'))+'</span>');
            } else if (percent == -1) {
    ...
    drawList: function(){
        ...
        Ext.each(ds.getAll(),function(record){
            ...
            html.push('<div class="up_card" id="atm_'+record.id+'">');
            html.push('<div class="icon icon-'+type+'"> </div>');
            html.push('<div class="j"><a target="_self" '+(atmTypeRef(record.get("fileName")) ? 'ref="img"' : '') +' href="'+this.downloadurl+'?attachmentId='+record.get('attachmentId')+'&accessId='+record.get('accessId')+'&tableName='+record.get('tableName')+'&tablePkValue='+record.get('tablePkValue')+'\" title="'+record.get("fileName")+'">'+record.get('fileName')+'</a></div>');

            html.push('<div class="l">');

            // 控制删除按钮的显示和隐藏
            debugger
            this.showdelete = record.get('showDelete');
            ...
            html.push('<div class="k">'+formatFileSize(record.get('fileSize'))+'<span class="d">'+Aurora.formatDateTime(record.get('creationDate'))+'</span></div>');
            html.push('</div>')            
        },this)
