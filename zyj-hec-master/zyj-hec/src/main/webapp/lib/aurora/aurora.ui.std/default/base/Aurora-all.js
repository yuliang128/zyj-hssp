...
// var isAdd = r.dirty; //MAS云新增特性
            ...
            d['__id'] = r.id;
            d['__status'] = r.isNew ? 'insert' : 'update';
            if (isAdd || selected) {
                datas.push(d);
            }
        });
...
// },
        ...
        fetchRecord: function () {
            ...
            if (!Ext.isEmpty(svc)) {
// url = sf.context +
// 'sys_lov.svc?svc='+sf.lovservice+'&pagesize=1&pagenum=1&_fetchall=false&_autocount=false&'+
...
//                 url = ctx + 'sys_lov.screen?url=' + encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/' + svc + '/query', Ext.urlEncode(sf.getLovPara()))) + '&service=' + svc;
                url = ctx + 'common/auroraLov/' + lovcode;
            }
            if (url) {
                sf.isWinOpen = true;
                sf.win = new A.Window({
    ...
    A.Tledit = Ext.extend($A.TextField, {
        ...
        processValue: function (v) {
            var sf = this,
                ...
                currentLang = document.cookie.match(new RegExp("(^| )" + "LANG" + "=([^;]*)(;|$)"))[2];
        ...
        commit: function (r, currentLang) {
            var sf = this,
                fieldName = sf.getBindName(),
                field_multi = fieldName + '_multi_cache';
            record = sf.record;
...
//    			debugger;
        ...
        showTleditWindow: function () {
            ...
            var sf = this,
                ...
                multi_cache = record ? (JSON.stringify(record.get(field_multi)) || '') : '',
                url = ctx + 'sys_tledit.screen?description_id=' + description_id + '&multi_cache=' + encodeURIComponent(multi_cache) + '&current_value=' + encodeURIComponent(record ? (record.get(fieldName) || '') : '');
...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
            ...
            fieldRecord = encodeURIComponent(JSON.stringify(fieldRecord));
            if (showCode) {
                url = ctx + 'sys_multiLov.screen?url=' + encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/' + svc + '/query', Ext.urlEncode(sf.getLovPara()))) + '&service=' + svc + '&fieldRecord=' + fieldRecord + '&showCode=' + showCode;
            } else {
                var mapps = sf.getMapping();
                var showCode = mapps[1].from;
                url = ctx + 'sys_multiLov.screen?url=' + encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/' + svc + '/query', Ext.urlEncode(sf.getLovPara()))) + '&service=' + svc + '&fieldRecord=' + JSON.stringify(fieldRecord) + '&showCode=' + showCode;
            }
        ...
        if (url) {
            sf.isWinOpen = true;
            sf.win = new $A.Window({
                title: sf.title || 'multiLov',
                url: Ext.urlAppend(url, "lovid=" + sf.id + "&key=" + encodeURIComponent(v) + "&gridheight=" + (sf.lovgridheight - 60 || 280) + "&innerwidth=" + (w - 30) + "&innergridwidth=" + Math.round((w - 90) / 2) + "&lovautoquery=" + (Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery) + "&lovlabelwidth=" + (sf.lovlabelwidth || 75) + "&lovpagesize=" + (sf.lovpagesize || '')),
                height: h,
                width: w
            });
            sf.win.on('close', sf.onWinClose, sf);
