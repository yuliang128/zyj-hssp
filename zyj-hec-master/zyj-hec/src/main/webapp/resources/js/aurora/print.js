Aurora.Printer = function(){
	...
	var processTextArea = function(textArea){
        Ext.DomHelper.insertBefore(textArea.wrap,'<span class="prt">'+textArea.wrap.dom.value.replace('<table border="0"','<table border="1"')+'</span>');
        textArea.wrap.addClass('prt_h');
    
    }
    var processRadio = function(radio){
        var cw = radio.wrap.child('div.item-radio-img-c')
        // 增加radiao只读判断 20160511
        if(!cw){
        	cw = radio.wrap.child('div.item-radio-img-readonly-c');
        }
        if(cw){
    ...
    var processCheckBox = function(cb){
        ...
        // 增加checkBox只读判断 20160511
        if(!cw){
        	cw = cb.wrap.child('div.item-ckb-readonly-c');
        }
        if(cw){
    ...
    var convertGridToTable = function(grid,isClear){
        ...
        for(var i=0;i<heads.length;i++){
        	  if (parseInt(heads[i].getAttribute('colspan')) != 1){
        	  	h = h + parseInt(heads[i].getAttribute('colspan'));
        	  }
        }
        for(var i=0;i<heads.length-h;i++){
        	  for (var j1 = 0;j1<parseInt(heads[i].getAttribute('colspan'));j1++){
        	  	if (parseInt(heads[i].getAttribute('colspan')) != 1){
        	  		var inn = heads[i].innerHTML + heads[heads.length-h+h1+j1].innerHTML;
        	    }else{
        	  		var inn = heads[i].innerHTML;
        	  	}
        	  ...
        	  if (parseInt(heads[i].getAttribute('colspan')) != 1){
        	  	h1 = h1 + parseInt(heads[i].getAttribute('colspan'));
        	  }
    ...
    return {
         doPrint : function(){
          ...
          // rtf
            var ke = Ext.DomQuery.select(".ke-container");
            if(ke.length >0){
