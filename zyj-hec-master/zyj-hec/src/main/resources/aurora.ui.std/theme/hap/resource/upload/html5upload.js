...
$A.HTML5Uploader = Ext.extend($A.Component,{
    ...
    uploadFiles : function(files){
        ...
        for (var i = 0; i < files.length; i++) {  
            ...
            Ext.each($au(this.id+'_ds').data,function(item){
            ...
            var record = new Aurora.Record({
                ...
                fileName : file.name,
                fileSize : file.size,
                tableName: this.sourcetype,
                tablePkValue : this.pkvalue,
                percent: 0,
                sequence:_s
            });
            $au(this.id+'_ds').add(record,0);
    ...
    uploadProgress: function(evt,fid) {
        if (evt.lengthComputable) {
            try {
                ...
                var record = $au(this.id+'_ds').find('fileId', fid);
                if (record) {
    ...
    uploadComplete: function(evt, fid) {
        if(evt.target.status == '200') {
            ...
            }else if(!isNaN(res.attachmentId)){
                if (record) {
...
$A.ResumableUploader = Ext.extend($A.HTML5Uploader,{
    ...
    initComponent : function(config){
        ...
        r.on('fileAdded', function(file){
            if(file.fileName){
                Ext.each($au(self.id+'_ds').data,function(item){
                    var s = item.get('sequence');
                ...
                file.fid = fid;
                var record = new Aurora.Record({
                    fileId : fid,//file.uniqueIdentifier,
                    fileName : file.fileName,
                    fileSize : file.size,
                    tableName: self.sourcetype,
                    tablePkValue : self.pkvalue,
                    percent: 0,
                    sequence:_s
                });
                $au(self.id+'_ds').add(record,0);
            }
        ...
        r.on('fileSuccess', function(file,message){
            if(file instanceof ResumableChunk){
                var record = $au(self.id+'_ds').find('fileId', file.fileObj.fid);
                if (message && message.substr(0,8) == 'Finished' && record) {
                    var res = message.split('-')[1];
                    var json = Ext.decode(res);
                    record.set('attachmentId',json.attachmentId);
                    record.set('accessId',json.accessId);
                    record.set('status', 1);
                    record.set('creationDate',new Date())
                    record.commit();
