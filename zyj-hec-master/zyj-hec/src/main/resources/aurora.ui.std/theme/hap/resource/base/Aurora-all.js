...
//		this.monthSpan[ou]("click",this.onViewShow,this);
	...
	onSelect:function(e,t){
		...
		if(td.hasClass("item-day") && !td.hasClass("item-day-besides")){
            ...
            sf.win = new $A.Window({title:sf.title||'calendar',fullScreen:true, url:Ext.urlAppend(url,"lovid="+sf.id+"&lovautoquery="+(Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery))});
...
A.showOkCancelWindow = function(title, msg, okfun,cancelfun,width, height){
    ...
    // if(cmp == null) {
       ...
       /* width = width||400;
        ...
        cmp = new A.Window({id:'aurora-msg-ok-cancel'+id,closeable:true,title:title, height:height||120,width:width||400,isDialog:true});
...
A.showYesNoCancelWindow = function(title, msg, yesfun,nofun,width, height){
    ...
    // if(cmp == null) {
        ...
        var id = Ext.id(),
	        ...
	        cancelbtnhtml = A.Button.getTemplate(cancelid,_lang['window.button.cancel']),
        	cmp = new A.Window({id:'aurora-msg-yes-no-cancel'+id,closeable:true,title:title, height:height||120,width:width||300});
        if(!Ext.isEmpty(msg,true)){
            msg = '<div class="win-icon win-question"><div class="win-type" style="width:'+(width-70)+'px;max-height:'+(height-55)+'px;">'+msg+'</div></div>';
...
A.showOkWindow = function(title, msg, width, height,callback){
    ...
    // if(cmp == null) {
        ...
        cmp = new A.Window({id:'aurora-msg-ok'+id,closeable:true,title:title, height:height,width:width,isDialog:true});
        if(!Ext.isEmpty(msg,true)){
...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
        ...
        if(url) {
	        sf.isWinOpen = true;                                                                                                                                                                                                                                                       
            sf.win = new A.Window({title:sf.title||'Lov', url:Ext.urlAppend(url,"lovid="+sf.id+"&key="+encodeURIComponent(v)+"&gridheight="+Math.max(lgh||355,355)+"&innerwidth="+(w-15)+"&lovautoquery="+(Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery) +"&lovlabelwidth="+Math.max(sf.lovlabelwidth||110,110)+"&lovpagesize="+(sf.lovpagesize||'')), height:h,width:w});
            sf.win.on('close',sf.onWinClose,sf);
        }
    },
    isEventFromComponent:function(el){
...
//    			debugger;
    ...
    showTleditWindow : function(){
    	...
    	sf.isWinOpen = true;
    	sf.win = new A.Window({title:sf.title||'输入多语言信息',url:Ext.urlAppend(url,"tleditid="+sf.id+"&margin="+margin),width:width,height:height});
    	sf.win.on('close',sf.onWinClose,sf);
    }
});
/**
...
// Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
        ...
        if(url) {
	        sf.isWinOpen = true;
            sf.win = new $A.Window({title:sf.title||'multiLov', url:Ext.urlAppend(url,"lovid="+sf.id+"&key="+encodeURIComponent(v)+"&gridheight="+(sf.lovgridheight-60||280)+"&innerwidth="+(w-30)+"&innergridwidth="+Math.round((w-90)/2)+"&lovautoquery="+(Ext.isEmpty(sf.lovautoquery) ? 'true' : sf.lovautoquery)+"&lovlabelwidth="+(sf.lovlabelwidth||75)+"&lovpagesize="+(sf.lovpagesize||'')), height:h,width:w});
            sf.win.on('close',sf.onWinClose,sf);
        }
    },
//    clearBind : function(){
