...
(function ($) {
	...
	win.AuroraAdapter = win.AuroraAdapter || ($ && {
		...
		removeEvent: function (el, eventType, handler) {
			...
			$au(el).unbind(eventType, handler);
		},
...
var rgbaRegEx = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/,
	hexRegEx = /$au([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/,
...
//    	}else{
			...
			for(var j=0;j<sf.yAxis.length;j++){
				...
				if(yAxis.bindtarget){
					for(var i=0,ds2 = $au(yAxis.bindtarget),rs = ds2.getAll(),l = rs.length,name = yAxisNames[0],yAxisNames=[];i<l;i++){
